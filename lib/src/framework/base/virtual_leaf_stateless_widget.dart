  ...
  Widget toWidget(RenderPayload payload) {
    try {
      final extendedContext = payload.observabilityContext
          ?.extendHierarchy([refName ?? runtimeType.toString()]);

      return ObservabilityScope(
        value: extendedContext,
        child: Builder(
          builder: (newContext) {
            // Create updated payload that uses the new context with extended hierarchy
            final updatedPayload = payload.copyWith(buildContext: newContext);

            if (commonProps == null) return render(updatedPayload);
            final isVisible = commonProps?.visibility
                    ?.evaluate(updatedPayload.scopeContext) ??
                true;
